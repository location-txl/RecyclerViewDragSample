 ...
 * {@link TestItemTouchHelper.Callback} class or implementing {@link TestItemTouchHelper.ViewDropHandler}
 ...
 */
public class TestItemTouchHelper extends RecyclerView.ItemDecoration
     ...
     */
    public TestItemTouchHelper(@NonNull Callback callback) {
        mCallback = callback;
    }
    ...
    void select(@Nullable ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                getSelectedDxDy(mTmpPosition);
                float[] applyTargetTranslate = mCallback.applyTargetTranslate(mRecyclerView, prevSelected, targetTranslateX, targetTranslateY);
                targetTranslateX = applyTargetTranslate[0];
                targetTranslateY = applyTargetTranslate[1];
     ...
     * {@link TestItemTouchHelper.Callback#isLongPressDragEnabled()}.
     * <p>
     ...
     * ItemTouchHelper
     * is attached.</li>
     * <li>{@link TestItemTouchHelper.Callback} must have dragging enabled.</li>
     ...
     *                   RecyclerView.
     * @see TestItemTouchHelper.Callback#isItemViewSwipeEnabled()
     */
     ...
     * behavior
     * by overriding {@link TestItemTouchHelper.Callback}
     * <p>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemTouchHelper is attached.</li>
     * <li>{@link TestItemTouchHelper.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
     ...
     * {@link TestItemTouchHelper}.
     */
    public interface ViewDropHandler {

        /**
         * Called by the {@link TestItemTouchHelper} after a View is dropped over another View.
         * <p>
    ...
    public abstract static class Callback {
         ...
         * @return 是否是内部 view
         */
        protected boolean getTargetRect(ViewHolder viewHolder, Rect outRect){
            outRect.set(
        ...
        public ViewHolder chooseDropTarget(@NonNull ViewHolder selected,
            ...
            Log.d("txlA", "dx:" + dx + " dy:" + dy);
            for (int i = 0; i < targetsSize; i++) {
                final ViewHolder target = dropTargets.get(i);
                isInnerView = getTargetRect(target, targetRect);
//                Log.d("txlA", " isInnerView:" + isInnerView);
                if (dx > 0) {
                    //原控件右边
                    int diff = targetRect.right - right;
                ...
                if (dy < 0) {
                    int diff = targetRect.top - curY;

                    if (diff > 0
                            && (
                                    !isInnerView
                                            ||
                                            targetRect.right >= selected.itemView.getRight()
                                )
                            && targetRect.top < selected.itemView.getTop()) {
                        final int score = Math.abs(diff);
                        if (score > winnerScore) {
                            Log.d("txlA", " dy < 0");
                            winnerScore = score;
                ...
                if (dy > 0) {
                    int diff = targetRect.bottom - bottom;

                    if (diff < 0 && targetRect.bottom > selected.itemView.getBottom()) {
                        final int score = Math.abs(diff);
                        if (score > winnerScore) {
                            Log.d("txlA", " dy > 0");
         ...
         *                    it is cleared.
         * @param actionState One of {@link TestItemTouchHelper#ACTION_STATE_IDLE},
         *                    {@link TestItemTouchHelper#ACTION_STATE_SWIPE} or
         *                    {@link TestItemTouchHelper#ACTION_STATE_DRAG}.
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
                List<TestItemTouchHelper.RecoverAnimation> recoverAnimationList,
                int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
                final TestItemTouchHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
